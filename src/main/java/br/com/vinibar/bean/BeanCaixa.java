...
public class BeanCaixa {
	...
	double totalItens;
	int idc = 0, pegaIditem = 0; // idc: variável local para pegar a id da
									// comanda.
	String status;
	...
	public void addItem() {
		...
		comanda.setIdcomanda(idc);
		if (itenscomanda.getIdproduto() == null) { // verifica se um item foi
													// selecionado no combo
			msg.error("SELECIONE UM ITEM");
		} else {
			if (itenscomanda.getIditenscomanda() == null) { // verifica seleção
															// de item via
															// tabela
	...
	public void salvar() {
		...
		if ("SELECIONE".equals(caixa.getFormapagamento())) {
			...
			new DaoComanda().Update(status, caixa.getIdcomanda());
